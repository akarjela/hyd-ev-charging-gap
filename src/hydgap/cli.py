...
def build(config: Path = ConfigOpt, raw: Path = RAW, out: Path = PROCESSED) -> None:
    ...
    line = f"stations ({s['source']}): {s['kept']} kept, {s['dropped_no_coords']} rows without coordinates, {s['unknown_status']} unknown status"
    if "duplicates_dropped" in s:
        line += f", {s['duplicates_dropped']} CSV rows within {s['dedupe_m']:.0f} m of an OCM station dropped"
    typer.echo(line)
